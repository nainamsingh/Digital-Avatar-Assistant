...
import os
os.environ['SPEECH_REGION'] = 'YOUR_SPEECH_REGION_HERE'
os.environ['SPEECH_KEY'] = 'YOUR_SPEECH_KEY_HERE'
os.environ['AZURE_OPENAI_ENDPOINT'] = 'https://instdigitalavataropenai.openai.azure.com/'
...
print("SPEECH_KEY:", os.environ.get('SPEECH_KEY', '')[:5] + '...' if os.environ.get('SPEECH_KEY') else None)
print("AZURE_OPENAI_ENDPOINT:", os.environ.get('AZURE_OPENAI_ENDPOINT'))
print("AZURE_OPENAI_API_KEY:", os.environ.get('AZURE_OPENAI_API_KEY', '')[:5] + '...' if os.environ.get('AZURE_OPENAI_API_KEY') else None)
...
def query_llm(prompt, model="hugging-quants/Llama-3.2-1B-Instruct-Q8_0-GGUF"):
    payload={
        "model":model,
...
# The chat route, which shows the map web page
@app.route("/map")
def mapView():
    return render_template("map.html", methods=["GET"], client_id=initializeClient())
...
def handleWsMessage(message):
    ...
    if path == 'api.audio':
        ...
        if not chat_initiated:
            ...
            client_context['chat_initiated'] = True
        user_query = "Hi, how are you!" #message.get('userQuery')
        for chat_response in handleUserQuery(user_query, client_id):
            socketio.emit("response", { 'path': 'api.chat', 'chatResponse': chat_response }, room=client_id)
...
def refreshIceToken() -> None:
    ...
    # Check if speech credentials are available
    if (not speech_region or not speech_key) and not speech_private_endpoint:
        print("WARNING: Speech credentials not configured. Speech functionality will be disabled.")
        ice_token = "dummy-token-for-local-development"
        return
    ...
    try:
        ...
        if ice_token_response.status_code == 200:
            ...
            print(f"WARNING: Failed to get ICE token. Status code: {ice_token_response.status_code}")
            ice_token = "dummy-token-for-local-development"
    except Exception as e:
        print(f"WARNING: Error while getting ICE token: {str(e)}")
        ice_token = "dummy-token-for-local-development"
